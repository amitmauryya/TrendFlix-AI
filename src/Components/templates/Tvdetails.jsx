...
const Tvdetails = () => {
  ...
  <div className='mt-8 w-full flex '>
     ...
     <div>        
         <h1 className=' font-black text-6xl text-white'>{info.detail.name || info.detail.title || info.detail.original_name || info.detail.original_title} <span className=' ml-3 text-zinc-300 text-4xl'>({info.detail.first_air_date.split("-")[0]})</span>
          </h1>
      </div>
        ...
        {info.detail.vote_average && <div className=' font-semibold  flex justify-center items-center text-white w-[5vh] h-[5vh] rounded-full bg-yellow-600'>{(info.detail.vote_average*10).toFixed()}<sup>%</sup></div>
          ...
          <h1 className='text-blue-300 text-2xl'> Tv Translated</h1>
